...
export default function Home() {
  ...
  return (
    <main className="grid select-none grid-rows-layout overflow-y-auto overflow-x-hidden grid-cols-3 lg:grid-rows-12 p-6 lg:grid-cols-10 h-full w-full xl:overflow-hidden 2xl:p-20 gap-6">
      <IconsBg />
      ...
      <Skills position={`${ui.hidden ? 'opacity-0 transition-opacity duration-500 ease-in-out' : 'opacity-100 transition-opacity duration-500 ease-in-out'} row-start-12 row-end-[30] col-span-full row-span-4 lg:col-start-1 lg:col-span-3 lg:row-start-4 lg:row-span-full `} />
      ...
      <Navbar position={`row-start-11 col-span-full  lg:col-start-4 lg:col-span-4 lg:row-start-11 lg:row-span-full `} />
